...
function gainXP(amount) {
    ...
    updateHUD();
    try {
        localStorage.setItem('noirSave', btoa(JSON.stringify(playerState)));
    } catch(e) {}
    renderMap();
}
...
function loadState() {
    try {
        ...
        if (saved) {
            playerState = saved.startsWith('{') ? JSON.parse(saved) : JSON.parse(atob(saved));
            updateHUD();
            renderMap();
        }
    } catch (e) {
...
function saveGame() {
    try {
        let key = btoa(JSON.stringify(playerState));
        localStorage.setItem('noirSave', key);
        prompt("SYSTEM NOTIFICATION: Case progress backed up locally.\\n\\nYou can also copy your UNIVERSAL SAVE KEY below to load your progress on any device:", key);
    } catch (e) {
...
function loadGame() {
    try {
        let key = prompt("ENTER YOUR SECURE DECRYPTION KEY (Save Key)\\n\\nOr leave blank and click OK to load your Local Save:");
        if (key && key.trim() !== "") {
            playerState = JSON.parse(atob(key.trim()));
            localStorage.setItem('noirSave', key.trim());
            updateHUD();
            renderMap();
            alert("SYSTEM NOTIFICATION: Case files securely loaded from Universal Key.");
        } else {
            let saved = localStorage.getItem('noirSave');
            if (saved) {
                playerState = saved.startsWith('{') ? JSON.parse(saved) : JSON.parse(atob(saved));
                updateHUD();
                renderMap();
                alert("SYSTEM NOTIFICATION: Previous local case files successfully restored.");
            } else {
                alert("SYSTEM NOTIFICATION: No previous case files found.");
            }
        }
    } catch (e) {
        console.warn("Could not load save:", e);
        alert("ERROR: Invalid Decryption Key or corrupted save file.");
    }
}
...
window.startHack = (btn, topicId, challengeId) => {
    ...
    if (textarea.classList.contains('hidden')) {
        ...
        let isBypass = code === 'BYPASS';
        if (isBypass || (code.length > 5 && /[a-zA-Z0-9_]/.test(code))) {
            ...
            let successMessage = isBypass ? "SECURITY OVERRIDE ACCEPTED (NO XP GRANTED)" : "LOG DECRYPTED (+50 XP)";
            let nextBtnHtml = `<button class="mt-4 w-full bg-white/10 hover:bg-white hover:text-black border border-white/20 text-white transition-colors py-3 uppercase tracking-widest font-space text-sm" onclick="window.returnToMap()">>> PROCEED TO NEXT FILE</button>`;
            
            btn.parentElement.innerHTML = `<div class="text-noir-cyan font-space text-lg tracking-widest mt-2 flex items-center gap-2 uppercase animate-pulse"><span>✓</span> ${successMessage}</div>${nextBtnHtml}`;
        } else {
